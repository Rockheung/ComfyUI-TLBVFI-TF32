"""
VideoConcatenator V2 - Video Chunk Merging

Concatenates video-encoded chunks using FFmpeg concat demuxer (no re-encoding).
Works with ChunkVideoSaverV2 for efficient disk-based video processing.
"""
...
- Final step in video-encoded chunk workflow

🎯 Usage:
1. Enter session_id from ChunkVideoSaverV2
...
- No GPU needed (pure demuxing operation)
    ...
    def concatenate(self, session_id: str, output_dir: str = "",
        ...
        Args:
            session_id: Session identifier from ChunkVideoSaverV2
        ...
        """
        if not session_id:
            raise ValueError("session_id is required. Provide the session_id from ChunkVideoSaverV2.")
        ...
        print(f"\nVideoConcatenatorV2: Concatenating {len(chunks_info)} video chunks from {session_id}")
        ...
        print(f"VideoConcatenatorV2: Running FFmpeg concat demuxer...")
        ...
        print(f"VideoConcatenatorV2: Concatenation complete!")
        ...
        frames = None
        if return_frames:
            print(f"VideoConcatenatorV2: Loading video into memory...")
            frames = self._load_video_to_tensor(output_path)
            print(f"VideoConcatenatorV2: Loaded {frames.shape[0]} frames into memory")
        else:
            # Return dummy tensor
        ...
        if cleanup_chunks:
            print(f"VideoConcatenatorV2: Cleaning up chunks in {session_dir}")
