"""
ChunkVideoSaver V2 - Video-Encoded Chunks

Saves interpolated frames as H.264/H.265 encoded video chunks.
Much more disk-efficient than raw .pt files while maintaining concat compatibility.
"""
...
class ChunkVideoSaverV2:
    """
    Save interpolated frames as H.264/H.265 encoded video chunks.

    Much more efficient than raw tensors:
    - Raw .pt: ~1.5GB per chunk (9 frames @ 4K)
    - H.264 CRF18: ~50-100MB per chunk (10-30x smaller)
    - H.265 CRF23: ~30-50MB per chunk (30-50x smaller)
...
📌 Purpose:
- Encodes frames to H.264/H.265 for massive disk space savings
- 10-50x smaller than raw .pt files (~50-100MB vs 1.5GB @ 4K)
...
- 1800 chunks @ 4K:
  - Raw .pt: 2.7 TB
  - H.264 CRF18: 90-180 GB (15-30x smaller)
  - H.265 CRF23: 54-90 GB (30-50x smaller)
    """
    ...
    def save_chunk(self, frames: torch.Tensor, chunk_id: int, fps: int = 30,
        ...
        if not session_id:
            ...
            print(f"ChunkVideoSaverV2: Created new session {session_id}")
        ...
        print(
            f"ChunkVideoSaverV2: Saved chunk {chunk_id} → {chunk_path}\n"
